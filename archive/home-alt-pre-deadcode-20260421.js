document.addEventListener('DOMContentLoaded', () => {
    ...
    const workPills = Array.from(document.querySelectorAll('#work.home-work-archive--desktop-static .work-pill'));

    const applyWorkPillBreathing = () => {
        workPills.forEach((pill) => {
            const text = (pill.textContent || '').replace(/\s+/g, ' ').trim();
            const textLength = text.length;

            pill.classList.remove('is-pill-short', 'is-pill-medium', 'is-pill-long');

            if (textLength <= 5) {
                pill.classList.add('is-pill-short');
                return;
            }

            if (textLength <= 14) {
                pill.classList.add('is-pill-medium');
                return;
            }

            pill.classList.add('is-pill-long');
        });
    };

    applyWorkPillBreathing();
    ...
    const syncRailFocusState = () => {
        ...
        const anyCardOpen = workCards.some((card) => card.classList.contains('is-filing-open'));
        document.body.classList.toggle('home-alt--rail-focus', anyCardHovered || anyCardOpen);
    ...
    workCards.forEach((card) => {
        ...
        const enableDesktopCardInteraction = false;
        let filingCards = [];
        ...
        const openFilingCard = (card, idx) => {
            if (!enableDesktopCardInteraction || !isDesktopCarousel) {
                return;
            }
            ...
            carouselTrack.classList.add('has-filing-open');
        ...
        const closeFilingCard = () => {
            filingCards.forEach((card) => {
                card.classList.remove('is-filing-open');
            });
            ...
            filingActiveIdx = -1;
            carouselTrack.classList.remove('has-filing-open');
            resetFilingSpread();
            scheduleRailFocusSync();
        };
            ...
            const projCards = getDesktopOrderedCards(
                cards.filter((card) => !card.classList.contains('title-card'))
            );
        ...
        carouselContainer.addEventListener('mouseenter', (event) => {
            if (!isDesktopCarousel || !enableDesktopCardInteraction) {
                return;
            }
        ...
        carouselContainer.addEventListener('mousemove', (event) => {
            if (!isDesktopCarousel || !enableDesktopCardInteraction || !filingCards.length) {
                return;
            }
